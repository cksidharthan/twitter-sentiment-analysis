...
result_df <- data.frame(table(unlist(str_extract_all(books_to_read_tweet$text,"#[a-zA-Z0-9]{2,}"))))


write.csv(books_to_read_df,"bookstoread.csv")

books_to_read_tweet = read.csv("bookstoread.csv", header = T)
...
books_to_read_corpus <-tm_map(books_to_read_corpus, content_transformer(function(x) gsub(x, pattern = "c(", replacement = "freedom")))
...
books_to_read_corpus = tm_map(books_to_read_corpus, removeWords, c("bookstoread", "bookbloggers", "booklovers", "book", "books", "bookblogger", 
                                                                   "bookboost", "booklover"))
...
word_repeat_count = subset(word_count, word_count > 50)

# Barplot
barplot(word_count, las = 2, col = rainbow(50))
